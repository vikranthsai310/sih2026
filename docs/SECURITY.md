...
**Audited 2026-09-04 against `app-release-unsigned.apk`, 26.3 MiB, R8 minified; items 1
and 3 re-audited and their evidence restated 2026-09-06, when the Wi-Fi broadcast transport
added `INTERNET` and the first logging.** Eight of ten pass with evidence recorded
below. Two cannot be closed on this machine and say why. An item is not ticked because
somebody believes it; it is ticked because the line beside it can be re-run.
...
| 4 | No location permission; BLE scan is `neverForLocation` | **PASS** | `aapt2 dump permissions` on the release APK lists thirteen permissions. Neither `ACCESS_FINE_LOCATION` nor `ACCESS_COARSE_LOCATION` is among them, and `BLUETOOTH_SCAN` carries `usesPermissionFlags='neverForLocation'`. This is the check that item 3 used to be paired with, and it is unaffected by the `INTERNET` change: the count went from eleven to thirteen when `INTERNET` and `CHANGE_WIFI_MULTICAST_STATE` were added for the Wi-Fi broadcast transport, and no location permission was added at any point |
