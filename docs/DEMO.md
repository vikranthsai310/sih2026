...
| "Do all ten languages speak?" | Seven do. Tamil, Kannada and Odia recognise and display but do not synthesise, because no permissively licensed voice exists for them in any family sherpa-onnx packages — 642 published artefacts were checked. Meta MMS covers them and is CC-BY-NC, so it was refused rather than shipped quietly. Gujarati was in that set until 2026-09-06: Piper has no Gujarati, but a Mimic 3 voice trained on CMU Indic does exist under a licence granting use "for any purpose ... without fee", and it now ships. The two models that cover the remaining three permissively — `indic-parler-tts` at 880M parameters and `svara-tts-v1` at 3B — have no ONNX export and would not fit a 4 GB handset. A template-coded message still arrives spoken in all ten, because the table is held on every unit |
...
| "Is it really offline?" | Three checks, and all three can be run in this room. **Behaviour:** put both handsets in aeroplane mode. The Bluetooth path runs with the cellular radio off entirely; for Wi-Fi, leave aeroplane mode on and switch the hotspot back on. The round trip completes with no SIM, no tower and no router. **Code:** `grep -rnE "HttpURLConnection\|okhttp\|retrofit\|java\.net\.URL\|WebSocket\|openConnection" app/src/main core-*/src/main` returns nothing — there is no HTTP client of any kind in the application, and no dependency that could supply one. **Test:** `WifiBroadcastLinkTest` asserts that every address the Wi-Fi transport will send to is a broadcast address and nothing else, and that the limited broadcast is built from its four bytes rather than resolved from a name. The APK *does* declare `android.permission.INTERNET`, and we say so first rather than being asked: Android requires it to open any socket at all, including one that only ever addresses a broadcast address on the local subnet, and the problem statement itself asks for data "streamed through wifi/Bluetooth connected embedded device or another phone". `aapt2 dump permissions app-release.apk` prints **fourteen** lines: the thirteen `android.permission.*` entries this manifest declares, plus `org.itantra.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`, which is not a platform permission at all — AndroidX adds it automatically, it is signature-level, and the only app that can hold it is this one. No location permission is among them and `BLUETOOTH_SCAN` carries `neverForLocation`. Counted from a real build on 2026-09-06, because the previous answer here said "eleven" and would have been contradicted on stage |
