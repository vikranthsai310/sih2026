...
| **T-05** | No adequate open model exists for Odia | Low | Synthesis | **Largely resolved by verification.** Piper publishes Odia voices (Debjani, Manas), so the expected gap appears not to exist. Confirm against the official `rhasspy/piper-voices` repository in week 1; if confirmed, the Meta MMS dependency is dropped entirely and the CC-BY-NC disclosure with it | Odia voice absent from the official Piper repository | MITIGATING |
