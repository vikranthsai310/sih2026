...
| **T-04** | Total asset size defeats the efficiency criterion | **High** | Models | **Measured 2026-09-04, and the trigger has fired.** The release APK is **30.9 MB with no models in it at all** — arm64-only, minified, resources shrunk. `libonnxruntime.so` alone is 21.7 MB and the three sherpa libraries add 9.7 MB, so ~31 MB of the installer is native code that neither R8 nor resource shrinking touches. Options, cheapest first: try the `sherpa-onnx-static-link-onnxruntime` release variant, which may strip unreferenced ONNX operators; build sherpa from source with only the operators IndicConformer and VITS need; or revise N2 upward and say why. **A target quietly missed is worse than a target openly revised** | Installer above 30 MB, or any pack above 70 MB — **fired** | **OPEN** |
| **T-05** | **No open synthesis voice exists for three of the ten languages** | **High** | Synthesis | **Re-escalated 2026-09-04, and the gap is wider than Odia.** The `rhasspy/piper-voices` repository tree carries **no Tamil, Gujarati, Kannada or Odia** directory, and `piper/VOICES.md` corroborates it. An earlier note recorded Odia voices named Debjani and Manas as present; that came from a search summary rather than the repository and **was wrong**. Piper covers six of our ten: en, hi, bn, mr, te, ml. **Partly resolved 2026-09-06: Piper is not the only permissive family.** Every family sherpa-onnx packages was searched — 642 published artefacts across VITS, Piper, Mimic 3, Coqui, Matcha and Kokoro — and exactly one covers a language Piper misses: a **Mimic 3 VITS voice for Gujarati** trained on CMU Indic, whose licence grants use "for any purpose ... without fee". It now ships, so **seven of ten speak**. Tamil, Kannada and Odia are genuinely uncovered: the only permissive models that reach them, `indic-parler-tts` (880M) and `svara-tts-v1` (3B), have no ONNX export and will not fit a 4 GB handset, and espeak-ng — which can speak all three and is already statically linked — exports no synthesis entry point from `libsherpa-onnx-jni.so`, so it cannot be called without rebuilding sherpa-onnx from source. `models/manifest.json` declares `"tts": null` for those three and the interface still offers them (W4.6). Remaining options in order of preference: a Coqui VITS voice trained on AI4Bharat IndicTTS data (MPL-2.0, ~2 GPU-days per language); a sherpa-onnx rebuild exposing espeak synthesis, which would make all ten speak at formant quality; Meta MMS as a disclosed CC-BY-NC stopgap; or ship the three recognise-only and say so | Three languages still silent at the week-6 gate | **OPEN, narrowed** |
...
| **T-09** | Wi-Fi Direct discovery unreliable across manufacturers | Med | Transport | Since 2026-09-08 the group is formed automatically (`WifiDirectGroup`, TRANSPORT.md §4) and the hotspot path is the fallback for a build that discovers nobody. The road is the same UDP broadcast on both, so the fallback is a Settings toggle rather than a code path. Bluetooth runs alongside regardless | — | MITIGATING |
